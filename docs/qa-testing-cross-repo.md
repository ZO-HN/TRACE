...
For when both repos are open in the same chat/session so the full trainee ↔ coach loop can be exercised end-to-end. Everything below assumes the migrations in `supabase/migrations/` through `20260810000001_coach_workspace_features.sql` are applied (confirmed live as of 2026-08-11 — see `docs/TODO-needs-user-input.md`).

Both apps hit the same Supabase project (`lfaxkrorjljdeefnafjb`) with independent sessions per device/app — RLS is what scopes visibility, not which app is open. See `docs/client-app-contract-check-ins-exercises.md` and `docs/client-app-handoff-2026-08-10.md` for the write-direction contracts referenced throughout this doc.
...
If a cross-repo test expects a trainee to read `roadmaps` or `meal_plans` (e.g. "client views their assigned meal plan in the app"), that's a **missing feature**, not a bug — RLS currently blocks it entirely. Flag it rather than assuming it should already work; see `docs/client-app-handoff-2026-08-10.md` section 5.
