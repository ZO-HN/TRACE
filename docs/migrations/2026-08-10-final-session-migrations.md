...
- `supabase/migrations/20260810000000_feedback_and_notifications.sql` — adds two tables:
  - `public.feedback` — header "Feedback" submissions. Written by `src/hooks/useFeedback.ts`.
  - `public.notifications` — backs the header bell dropdown. Read/written by `src/hooks/useNotifications.ts`. Nothing inserts into it automatically (no triggers) — shows "No notifications yet" until rows exist.
- `supabase/migrations/20260810000001_coach_workspace_features.sql` — the big one. Adds tables for every page that was previously a UI-shell mock:
  ...
  All coach-owned tables (everything except `form_checks`) get the same RLS shape as `check_in_templates`/`feedback`: `coach_id = auth.uid()`, full CRUD on own rows. `form_checks` follows the `check_ins` RLS pattern (trainee inserts/reads own, coach reads/reviews own). `form_checks` is added to the realtime publication.
...
- Header → Feedback → topic + message → Send succeeds, row appears in `public.feedback`.
...
- Actually emailing `feedback` rows, or sending real client-invite emails, needs a Supabase Edge Function + an email provider secret (Resend/SendGrid/etc.) — not decided yet.
