# TRACE cross-repo QA guide

For the chat where `TRACE` (coach dashboard) and `TRACE-client` (trainee app) are both open together. Coach-dashboard-only TODOs live separately in `docs/handoff-todo.md` — this file only has what's needed for testing across both repos, present and future.
...
```
The `exercise:exercises(name)` embed relies on PostgREST auto-detecting the single FK from `form_checks.exercise_id` to `public.exercises(id)` (defined inline, no explicit constraint name, in `20260810000001_coach_workspace_features.sql`). This *should* resolve fine since there's only one FK path between those two tables — but it hasn't been smoke-tested against the live schema. If the join silently resolves to `null` instead of erroring, form checks will show "General form check" for every row instead of the actual exercise name.
